...
//insertar la cita
 function insertarCita($idUsuario, $fecha, $hora, $contador){
 $link=conexion();   
 $atendido=0;
...
return $salida;
 ...
 function eliminarCita($idUsuarioMisCitas, $idcita){
 $link=conexion(); 
...
return $salida;
 ...
 //llamada al metodo consultaHorario
  ...
  echo cargarAnimales($_POST['animales']);
} 
   if(isset($_POST['idUsuario']) && ($_POST['fecha']) && ($_POST['hora']) && ($_POST['contador'])) {
  echo insertarCita($_POST['idUsuario'], $_POST['fecha'], $_POST['hora'], $_POST['contador']);
} 
...
if(isset($_POST['idUsuarioMisCias'])) {
  echo misCitas($_POST['idUsuarioMisCias']);
} 
if(isset($_POST['idUsuarioMisCitasEliminar']) && ($_POST['idcita'])) {
  echo eliminarCita($_POST['idUsuarioMisCitasEliminar'], $_POST['idcita']);
} 
...
function verificarUsuario($contra, $correo){
  $link=conexion();
  $salida= ""; 
  $result = mysqli_query($link, "SELECT * FROM usuario WHERE correo = '".$correo."' AND contraseÃ±a = '".$contra."'"); 
  if($result!=NULL){
...
//paBuscarCatidadProductosCarrito
      $salida= $salida."<div class= \"contenedor\"><div class=\"producto\"><li class=\"galeria__item\"><img src=".$row['imagen']." class=\"galeria__img\" type=\"submit\"></li><span>".$row['nombre']."</span></div><p class=\"precio\">Precio: â‚¡".$row['precioUnitario']."</p> <p class=\"stock\">Stock: ".$row['stock']."</p><p class=\"cantL\">Cantidad:</p><select class=\"cantidad2\" id=\"cant\"><option>".$cant."</option>";
